...
UniformRigidBodyGenerator::UniformRigidBodyGenerator(WorldManager* s, string n, Vector3 p, Vector3 v, double iniS, double rndV, double rndP, double rndS, PxPhysics* gP,double tBS, double nS):
	name(n), worldManager(s), pos(p), vel(v), randomSize(rndS), randomPos(rndP), randomVel(rndV), iniSize(iniS), gPhyscis(gP), timeBtwSpawn(tBS), nSpawn(nS)
{
	currentTime = 0;
	lastSpawn = 0;
}
